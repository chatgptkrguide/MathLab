...
class ProblemRepository {
  ...
  Future<List<Problem>> loadProblemsByCategory(String category) async {
    ...
    if (category == 'polynomials') {
      try {
        final problem = await loadProblem(
            'assets/problems/polynomials/polynomial_001.json');
  ...
  Future<List<Problem>> loadAllProblems() async {
    try {
      ...
      // 고등학교 문제 로드
      final highSchoolProblems =
          await _loadGradeProblems('high', '고1', ['grade1']);
      ...
      if (allProblems.isEmpty) {
        ...
        Logger.debug(
            '[ProblemRepository] 총 ${allProblems.length}개 문제 로드 (학년별 폴더)');
      }
  ...
  Future<List<Problem>> _loadGradeProblems(String schoolLevel,
      String defaultGrade, List<String> gradesFolders) async {
    final problems = <Problem>[];
    ...
    for (final gradeFolder in gradesFolders) {
      try {
        ...
        final filePath =
            'assets/problems/$schoolLevel/$gradeFolder/polynomials.json';
        ...
        final loadedProblems = problemsJson.map((problemData) {
          ...
          return Problem(
            ...
            title: (data['chapter'] as String?) ??
                (data['category'] as String? ?? fileChapter ?? '문제'),
            ...
            choices: (data['options'] as List<dynamic>?)
                    ?.map((e) => e as String)
                    .toList() ??
                [],
        ...
        Logger.debug(
            '[ProblemRepository] $schoolLevel/$gradeFolder에서 ${loadedProblems.length}개 문제 로드');
      } catch (e) {
        // 파일이 없으면 스킵 (정상 동작)
        Logger.debug(
            '[ProblemRepository] $schoolLevel/$gradeFolder 문제 파일 없음 (스킵)');
      }
    }
  ...
  Future<List<Problem>> loadProblemsByLesson(String lessonId) async {
    ...
    if (filteredProblems.isNotEmpty) {
      Logger.debug(
          '[ProblemRepository] 레슨 $lessonId에 ${filteredProblems.length}개 문제 로드 (학년별 폴더)');
    ...
    try {
      ...
      if (koreanProblems.isNotEmpty) {
        Logger.debug(
            '[ProblemRepository] 한국 교육과정 문제 ${koreanProblems.length}개 로드: $lessonId');
  ...
  Future<List<Problem>> loadProblemsByLessonAndGrade(
      String lessonId, String grade) async {
    try {
      ...
      if (filteredProblems.isNotEmpty) {
        Logger.debug(
            '[ProblemRepository] 레슨 $lessonId, 학년 $grade에 ${filteredProblems.length}개 문제 로드');
        return filteredProblems;
      }
      ...
      try {
        ...
        if (koreanProblems.isNotEmpty) {
          Logger.debug(
              '[ProblemRepository] 한국 교육과정 문제 ${koreanProblems.length}개 로드: $lessonId (학년: $grade)');
          return koreanProblems;
        }
      } catch (e) {
        Logger.debug('[ProblemRepository] 한국 교육과정 문제 로드 실패: $e');
      ...
      Logger.debug(
          '[ProblemRepository] 레슨 $lessonId, 학년 $grade에 적절한 문제가 없습니다. 준비중 메시지 표시');
      return [_generateSingleProblem(lessonId)];
    } catch (e) {
      Logger.debug(
          '[ProblemRepository] 문제 로드 오류 (레슨: $lessonId, 학년: $grade): $e');
      return [_generateSingleProblem(lessonId)];
    }
  }
  ...
  Future<List<Problem>> loadProblemsBySchoolLevel(
      SchoolLevel schoolLevel) async {
    final problems = <Problem>[];

    // 학교급별로 학년 폴더 결정
    final gradesFolders = _getGradeFoldersForSchoolLevel(schoolLevel);
    final schoolLevelCode =
        schoolLevel.pathCode; // 'elementary', 'middle', 'high'
    final defaultGrade = schoolLevel.grades.first; // 첫 학년을 기본값으로

    final loadedProblems =
        await _loadGradeProblems(schoolLevelCode, defaultGrade, gradesFolders);
    problems.addAll(loadedProblems);
  ...
  Future<List<Problem>> loadProblemsBySchoolLevelAndGrade({
    ...
    if (!schoolLevel.containsGrade(grade)) {
      Logger.debug(
          '[ProblemRepository] 학년 $grade은(는) ${schoolLevel.displayName}에 속하지 않습니다.');
      return [];
    }
  ...
  List<Problem> filterBySchoolLevel(
      List<Problem> problems, SchoolLevel schoolLevel) {
